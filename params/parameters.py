...
def parse_args():
    ...
    parser.add_argument("-P", "--probsfile", metavar="PROBSFILE", default="",
                        help="json file specifying some probatilites (e.g. json/probabilites/example_probs.json) "
                             "(see json/probabilites/ for examples)")
...
def get_probs_to_override(probs_param: str) -> Union[Dict[str, dict], None]:
    ...
    for prob_key, prob_dict in local_dict.items():
        import cnerator
        if not cnerator.probs.does_this_probability_exist(prob_key):
...
def parse_probabilites_specification_json_file(file_name: str) -> Union[Dict[str, dict], None]:
