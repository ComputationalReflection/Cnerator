...
available_lvalue_operators = defaultdict(set)
    ...
    for line in f:
        ...
        if m:
            ...
            try:
                ...
                assert error.message == "'dict' object has no attribute 'add'"
                by_operands[1][operator][op1_t] = set()
                by_operands[1][operator][op1_t].add(result_t)
            by_return[1][operator][result_t].add(op1_t)
            available_common_operators[1][result_t].add(operator)
...
for t in types:
    ...
    for result_t, operator, op1_t in data:
        ...
        available_common_operators[1][result_t].add(operator)
        if operator != "&":
            available_lvalue_operators[result_t].add(operator)
...
for arity in range(1, 4):
    for ret_t in sorted(types):
        if arity == 3:
            ...
            ops = available_common_operators[arity][ret_t]
            ops = ", ".join("'{}'".format(op) for op in sorted(ops))
        ret_t = camel_case_to_snake_case(ret_t)
        available_common_operators_parts.append("{}_{}_operators = [{}]".format(ret_t, arity, ops))
...
for ret_t in sorted(types):
    ...
    if arity not in available_common_operators or ret_t not in available_common_operators[arity]:
        ...
        ops = ", ".join("'{}'".format(op) for op in sorted(ops))
    ret_t = camel_case_to_snake_case(ret_t)
    available_assignment_operators_parts.append("{}_assignment_operators = [{}]".format(ret_t, ops))

available_lvalue_parts = []
for ret_t in sorted(types):
    ops = available_lvalue_operators[ret_t]
    ops = ", ".join("'{}'".format(op) for op in sorted(ops))
    ret_t = camel_case_to_snake_case(ret_t)
    available_lvalue_parts.append("{}_lvalue_operators = [{}]".format(ret_t, ops))
